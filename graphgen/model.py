...
class MLP_Softmax(nn.Module):
    """
    A deterministic linear output layer with Softmax.
    """
...
class MLP_Log_Softmax(nn.Module):
    """
    A deterministic linear output layer with LogSoftmax.
    """
    def __init__(self, input_size, embedding_size, output_size, dropout=0):
...
class MLP_Plain(nn.Module):
    """
    A basic MLP for transforming inputs.
    """
    def __init__(self, input_size, embedding_size, output_size, dropout=0):
...
class MLP_Regression(nn.Module):
    """
    A simple MLP regression output layer for node-level predictions.
    This head produces a single scalar per time step.
    """
...
class RNN(nn.Module):
    """
    Custom RNN layer (GRU or LSTM) for DFS-code generation.
    """
    def __init__(
        ...
        self.hidden = None  # Hidden state to be initialized in forward
    ...
    def forward(self, input, input_len=None):
        ...
        if input_len is not None:
            input_len = input_len.cpu().to(torch.int64)
...
def create_model(args, feature_map):
    ...
    dfs_code_rnn = RNN(
        ...
        embedding_size=args.embedding_size_dfscode_rnn,
        hidden_size=args.hidden_size_dfscode_rnn,
        num_layers=args.num_layers,
        rnn_type=args.rnn_type,
        dropout=args.dfscode_rnn_dropout,
        device=args.device
    ).to(device=args.device)

    output_timestamp1 = MLP_layer(
        input_size=args.hidden_size_dfscode_rnn,
        embedding_size=args.embedding_size_timestamp_output,
        output_size=max_nodes + 1,
        dropout=args.dfscode_rnn_dropout
    ).to(device=args.device)
    ...
    output_vertex1 = MLP_layer(
        input_size=args.hidden_size_dfscode_rnn,
        embedding_size=args.embedding_size_vertex_output,
        output_size=len_node_vec,
        dropout=args.dfscode_rnn_dropout
    ).to(device=args.device)
    ...
    if args.note == 'DFScodeRNN':
        output_edge = MLP_layer(
            input_size=args.hidden_size_dfscode_rnn,
            embedding_size=args.embedding_size_edge_output,
            output_size=len_edge_vec,
            dropout=args.dfscode_rnn_dropout
        ).to(device=args.device)
        model['output_edge'] = output_edge

    # Add an auxiliary regression head for node-level prediction.
    # This will output a single scalar per time step.
    output_level = MLP_Regression(
        input_size=args.hidden_size_dfscode_rnn,
        embedding_size=args.embedding_size_vertex_output,  # Alternatively, define a new hyperparameter if needed.
        output_size=1,
        dropout=args.dfscode_rnn_dropout
    ).to(device=args.device)
    model['output_level'] = output_level
