...
def produce_graphs_from_raw_format(
    ...
    """
    Process graphs from a raw-format file.
    """
    lines = []
    ...
    while index < len(lines):
        if lines[index][0][1:] not in graphs_ids:
            ...
            if not check_graph_size(G, min_num_nodes, max_num_nodes, min_num_edges, max_num_edges):
                continue
...
def produce_graphs_from_graphrnn_format(
    ...
    """
    Process graphs from GraphRNN-compatible files.
    Expected files:
      - {dataset_name}_A.txt
      - {dataset_name}_node_labels.txt
      - {dataset_name}_graph_indicator.txt
      - (optionally) {dataset_name}_graph_labels.txt
    """
    ...
    if node_attributes:
        data_node_att = np.loadtxt(os.path.join(path, dataset_name + '_node_attributes.txt'),
                                   delimiter=',')
    data_node_label = np.loadtxt(os.path.join(path, dataset_name + '_node_labels.txt'),
                                 delimiter=',').astype(int)
    data_graph_indicator = np.loadtxt(os.path.join(path, dataset_name + '_graph_indicator.txt'),
                                      delimiter=',').astype(int)
    if graph_labels:
        data_graph_labels = np.loadtxt(os.path.join(path, dataset_name + '_graph_labels.txt'),
                                       delimiter=',').astype(int)
    ...
    graph_num = int(data_graph_indicator.max())
    ...
    for i in range(graph_num):
        ...
        if not check_graph_size(G_sub, min_num_nodes, max_num_nodes, min_num_edges, max_num_edges):
            continue
        ...
        for node in G_sub.nodes():
            ...
            if 'CC' in node_invariants:
                node_label += '-' + str(bisect.bisect(cc_bins, clustering_coeff[node]))
...
def produce_amazon_graphs_from_csv(
    csv_file, output_path, num_graphs=None, min_num_nodes=None,
    max_num_nodes=None, min_num_edges=None, max_num_edges=None
):
    """
    Process graphs directly from the Amazon CSV.
    The CSV is expected to have at least the following columns:
      - "Initial_Node"
      - "Query"
      - "Suggestions"
    For each unique "Initial_Node", we build a graph.
    Here, we convert node labels to numeric values using a local mapping that starts at 1.
    """
    mkdir(output_path)
    data = pd.read_csv(csv_file)
    groups = data.groupby("Initial_Node")
    count = 0
    for group_key, group in groups:
        G = nx.Graph()
        node_map = {}    # Maps query/suggestion string -> local node ID
        label_map = {}   # Maps query/suggestion string -> unique numeric label
        current_label = 1  # Start numeric labels from 1
        node_id = 1
        for _, row in group.iterrows():
            query = str(row["Query"]).strip()
            if query not in label_map:
                label_map[query] = current_label
                current_label += 1
            if query not in node_map:
                node_map[query] = node_id
                G.add_node(node_id, label=label_map[query])
                node_id += 1
            suggestions = str(row["Suggestions"]).split(", ")
            for suggestion in suggestions:
                suggestion = suggestion.strip()
                if suggestion not in label_map:
                    label_map[suggestion] = current_label
                    current_label += 1
                if suggestion not in node_map:
                    node_map[suggestion] = node_id
                    G.add_node(node_id, label=label_map[suggestion])
                    node_id += 1
                G.add_edge(node_map[query], node_map[suggestion], label=0)
...
def produce_random_walk_sampled_graphs(
    ...
    with open(os.path.join(input_path, dataset_name + '.cites'), 'r') as f:
        for line in f.readlines():
            ...
            if spp[0] in d and spp[1] in d:
                G.add_edge(d[spp[0]], d[spp[1]], label=0)
            else:
                count += 1
    G.remove_edges_from(nx.selfloop_edges(G))
    G = nx.convert_node_labels_to_integers(G)
    with Pool(processes=48) as pool:
        for _ in tqdm(pool.imap_unordered(
                partial(sample_subgraphs, G=G, output_path=output_path,
    ...
    for i, name in enumerate(filenames[:num_graphs]):
        os.rename(os.path.join(output_path, name),
...
def create_graphs(args):
    if 'Lung' == args.graph_type:
        ...
        min_num_nodes, max_num_nodes = None, None
        min_num_edges, max_num_edges = 20, None
    elif 'amazon' in args.graph_type:
        base_path = os.path.join(args.dataset_path, 'amazon/')
        amazon_csv = os.path.join(base_path, "amazon_suggestions_graphrnn.csv")
        node_invariants = ['Degree']
        min_num_nodes, max_num_nodes = 1, 200
        min_num_edges, max_num_edges = 1, 1000
    else:
    ...
    if args.produce_graphs:
        ...
        if args.graph_type in ['Lung', 'Breast', 'Leukemia', 'Yeast', 'All']:
            ...
            print("Converting Amazon CSV to GraphRNN files with numeric labels...")
            conv_count = produce_amazon_graphs_from_csv(
                amazon_csv, args.current_dataset_path, args.num_graphs,
                min_num_nodes=min_num_nodes, max_num_nodes=max_num_nodes,
                min_num_edges=min_num_edges, max_num_edges=max_num_edges)
            count = conv_count
