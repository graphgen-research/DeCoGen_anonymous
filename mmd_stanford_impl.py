...
See: https://github.com/snap-stanford/GraphRNN
"""
...
def disc(samples1, samples2, kernel, is_parallel=False, *args, **kwargs):
    ...
    if not is_parallel:
        for s1 in samples1:
            for s2 in samples2:
                # print("kernel result:", kernel(s1, s2, *args, **kwargs))
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            for dist in executor.map(kernel_parallel_worker,
                ...
                d += dist
    #print("total_discrepancy:", d)
    #print("length:", len(samples1) * len(samples2))
